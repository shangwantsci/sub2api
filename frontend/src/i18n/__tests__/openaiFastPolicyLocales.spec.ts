...
describe('OpenAI Fast/Flex policy locale keys', () => {
  ...
  // Fork invariant: the user-scope copy must live on openaiFastPolicy, never on
  // the Anthropic betaPolicy namespace it was originally mis-nested under.
  it.each([
    ['en', en],
    ['zh', zh]
  ])('keeps user scope copy off the Anthropic beta policy (%s)', (_locale, messages) => {
