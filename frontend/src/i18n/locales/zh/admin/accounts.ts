export default {
    accounts: {
      ...
      dataExportConfirmMessage:
        '导出的数据包含账号与代理的敏感信息，请妥善保存。注意这不是完整备份：不含用户、分组定义、用量记录与结算单；账号归属与分组绑定按 id 记录，只在同一实例恢复时有效。需要完整备份请用系统设置里的整库备份。',
      ...
      dataImportWarning:
        '导入将创建新账号与代理。账号归属、分组绑定与启用状态会一并恢复，但归属和分组按 id 匹配，只在同一实例有效；换实例导入时对不上的分组会被跳过并在结果里列出。请确认已有数据不会冲突。',
      dataImportFile: '数据文件',
      ...
      persona: {
        ...
        timezoneHint: '默认按代理出口国家自动识别；也可直接选择东八区、新加坡、日本或美国常用时区。',
        locale: 'Locale',
        localeHint: '默认跟随代理国家/所选时区；仅作组织标记，不写入任何出站请求头。',
        customOption: '历史自定义值：{value}',
        timezoneOptions: {
          auto: '自动（根据代理出口国家，推荐）',
          china: '东八区（上海 / 北京，UTC+8）',
          singapore: '新加坡（UTC+8）',
          japan: '日本（东京，UTC+9）',
          usEastern: '美国东部（纽约，UTC-5 / UTC-4）',
          usCentral: '美国中部（芝加哥，UTC-6 / UTC-5）',
          usMountain: '美国山地（丹佛，UTC-7 / UTC-6）',
          usPacific: '美国西部（洛杉矶，UTC-8 / UTC-7）',
        },
        localeOptions: {
          auto: '自动（跟随代理国家 / 时区）',
          zhCN: '简体中文（zh-CN）',
