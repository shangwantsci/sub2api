export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        claudeOAuthBillableOverride: '注入量标定值（tokens）',
        claudeOAuthBillableOverrideHint: '填 0 用本地估算。本地用 o200k 分词，与 Anthropic 口径有系统性偏差（实测 identity_only 两块本地估 42、官方计 28）。标定方法：用官方 count_tokens 测出客户内容真值，填入「上游 input_tokens − 该真值」。注入内容形态固定，一次标定长期有效。',
        claudeOAuthBillableInputTokens: 'usage 扣除身份注入',
        claudeOAuthBillableInputTokensHint: '从回给客户端的 usage.input_tokens 中扣除网关注入的身份 blocks（约 41 tokens），使客户端只看到自己的输入。仅扣不带 cache_control 的块——带缓存的块计入 cache_creation/cache_read，本就不在 input_tokens 里。只改展示，计费与审计仍用上游原始 usage。默认关闭。',
        ...
        personaGating: '账号人格门控',
        personaGatingHint: '默认关闭。全局开关：开启后，对同时在账号里设置了 persona_enabled 的账号启用"人格信封"——按人格时区的作息窗口调度、并把并发压到人格上限。二者需同时为真才生效，故打开此全局开关不会影响任何未配置 persona 的现有账号（fail-open，行为与之前完全一致）。用于把池账号整形成"单人作息"以延长存活。',
      ...
      calibratedProfile: {
        title: 'Claude Code 标定 Profile（真身自动标定）',
        description: '由 tools/cc-calibrate 在服务器上运行真实 Claude Code CLI 抓包标定所得的 profile（请求头模板 + 分档 anthropic-beta + cc_version + 指纹守卫）。发布后网关热加载它覆盖内置出站字节，升级 CLI 只需重跑标定并重新发布，无需改代码。未发布或校验不通过时自动回退到内置常量。',
        active: '已生效',
        guardPassed: '指纹守卫通过',
        userAgent: 'User-Agent',
        capturedAt: '标定时间',
        source: '标定来源',
        sourceAutoHint: '（自动标定 sidecar）',
        refresh: '刷新',
        invalid: '已发布的 profile 校验未通过，未生效。',
        ...
        cleared: '已清除标定 profile，回退到内置常量。',
        emptyError: '请粘贴 profile JSON 内容',
        publishError: '发布失败：profile 未通过校验（schema/版本一致性/指纹守卫）',
        clearError: '清除失败'
      },
      betaPolicy: {
        title: 'Beta 策略',
