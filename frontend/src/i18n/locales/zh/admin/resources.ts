export default {
    ...
    proxies: {
      ...
      dataImportWarning:
        '导入将创建或复用代理，保留状态并在完成后自动触发延迟检测。复用已存在的代理时，归属与「允许自动分配」开关会保留线上现值、不被备份覆盖，差异会在结果里列出。',
      dataImportFile: '数据文件',
      ...
      dataExportConfirmMessage:
        '导出的数据包含代理的敏感信息，请妥善保存。注意这不是完整备份：不含账号、用户与结算单；代理归属按 id 记录，只在同一实例恢复时有效。',
      dataExportConfirm: '确认导出',
      dataExported: '数据导出成功',
      dataExportFailed: '数据导出失败',
      autoAssignEnabled: '已开放给供号商自动分配',
      autoAssignDisabled: '未开放自动分配',
      autoAssignProviderOwned: '供号商自带的代理不能开放共享',
      autoAssignFailed: '切换自动分配失败',
      columns: {
        name: '名称',
        protocol: '协议',
        address: '地址',
