export default {
    ...
    proxies: {
      ...
      dataImportWarning:
        'Import will create or reuse proxies, keep their status, and trigger latency checks after completion. When an existing proxy is reused, its ownership and auto-assign switch keep the current live values rather than being overwritten by the backup; any difference is listed in the result.',
      ...
      dataExportConfirmMessage:
        'The exported data contains sensitive proxy information. Store it securely. Note this is not a full backup: it excludes accounts, users and settlements, and proxy ownership is stored by id, so it only restores correctly on the same instance.',
      dataExportConfirm: 'Confirm Export',
      dataExported: 'Data exported successfully',
      dataExportFailed: 'Failed to export data',
      autoAssignEnabled: 'Open for automatic assignment to providers',
      autoAssignDisabled: 'Not open for automatic assignment',
      autoAssignProviderOwned: 'A provider-supplied proxy cannot be shared',
      autoAssignFailed: 'Failed to change automatic assignment',
      ...
      columns: {
        ...
        status: 'Status',
        accounts: 'Accounts',
        autoAssignable: 'Auto-assign',
        providerOwned: 'Provider {id}',
        latency: 'Latency',
        expiry: 'Validity',
        createdAt: 'Created',
        actions: 'Actions',
