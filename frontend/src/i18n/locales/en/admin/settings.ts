export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        personaGating: 'Account Persona Gating',
        personaGatingHint: 'Default off. Global switch: when on, accounts that ALSO set persona_enabled get their "persona envelope" enforced — scheduled within the persona timezone active-hours window and capped to the persona concurrency limit. Both must be true to take effect, so enabling this global switch does not affect any existing account without a persona configured (fail-open, behavior identical to before). Used to shape pool accounts into a single-person cadence to extend survival.',
      ...
      calibratedProfile: {
        title: 'Claude Code Calibrated Profile (real-CLI auto-calibration)',
        description: 'A profile captured by running the real Claude Code CLI on the server via tools/cc-calibrate (header template + per-bucket anthropic-beta + cc_version + fingerprint guard). Once published, the gateway hot-loads it to override the built-in outbound bytes; upgrading the CLI just means re-running calibration and re-publishing — no code changes. Falls back to compiled-in constants when unpublished or invalid.',
        active: 'Active',
        guardPassed: 'Fingerprint guard passed',
        userAgent: 'User-Agent',
        capturedAt: 'Captured at',
        source: 'Source',
        sourceAutoHint: '(auto-calibration sidecar)',
        refresh: 'Refresh',
        invalid: 'The published profile failed validation and is NOT active.',
        ...
        cleared: 'Calibrated profile cleared; falling back to built-in constants.',
        emptyError: 'Please paste the profile JSON content',
        publishError: 'Publish failed: the profile failed validation (schema / version consistency / fingerprint guard)',
