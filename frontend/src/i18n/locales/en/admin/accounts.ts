export default {
    accounts: {
      ...
      dataExportConfirmMessage:
        'The exported data contains sensitive account and proxy information. Store it securely. Note this is not a full backup: it excludes users, group definitions, usage records and settlements, and account ownership plus group bindings are stored by id, so they only restore correctly on the same instance. Use the database backup in system settings for a complete backup.',
      ...
      dataImportWarning:
        'Import creates new accounts and proxies. Ownership, group bindings and enabled state are restored too, but ownership and groups match by id and only hold on the same instance; groups that do not exist here are skipped and listed in the result. Ensure existing data does not conflict.',
      ...
      dataImportErrors: 'Failures and notices',
      ...
      persona: {
        ...
        timezoneHint: 'Defaults to auto-detection from the proxy exit country; common UTC+8, Singapore, Japan, and US zones are also available.',
        locale: 'Locale',
        localeHint: 'Defaults from the proxy country/selected timezone. Organizational metadata only; never sent as an outbound header.',
        customOption: 'Legacy custom value: {value}',
        timezoneOptions: {
          auto: 'Auto (from proxy exit country, recommended)',
          china: 'UTC+8 (Shanghai / Beijing)',
          singapore: 'Singapore (UTC+8)',
          japan: 'Japan (Tokyo, UTC+9)',
          usEastern: 'US Eastern (New York, UTC-5 / UTC-4)',
          usCentral: 'US Central (Chicago, UTC-6 / UTC-5)',
          usMountain: 'US Mountain (Denver, UTC-7 / UTC-6)',
          usPacific: 'US Pacific (Los Angeles, UTC-8 / UTC-7)',
        },
        localeOptions: {
          auto: 'Auto (from proxy country / timezone)',
          zhCN: 'Simplified Chinese (zh-CN)',
          enUS: 'US English (en-US)',
          enSG: 'Singapore English (en-SG)',
          jaJP: 'Japanese (ja-JP)',
        },
