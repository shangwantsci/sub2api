...
func TestAccountRepository_ListOAuthRefreshCandidatePage_SQLFilter(t *testing.T) {
	...
	require.Contains(t, normalized, "credentials->>'oauth_client' = 'claude_chrome'")
	require.Contains(t, normalized, "credentials ? 'session_key'")
	require.Contains(t, normalized, "btrim(credentials->>'session_key') <> ''",
		"Claude Chrome accounts with a sessionKey must remain eligible for fallback even when refresh_token is missing")
