...
func (h *GatewayHandler) ChatCompletions(c *gin.Context) {
	...
	if decision := h.contentSafetyDecision(c, reqLog, apiKey, subject, service.ContentModerationProtocolOpenAIChat, reqModel, body); decision != nil && decision.Blocked {
		h.chatCompletionsErrorResponse(c, contentSafetyStatus(decision), contentSafetyErrorCode(decision), decision.Message)
		return
	}
	...
	fs := h.newFailoverState(h.maxAccountSwitches, false, groupPlatform)
