...
type GatewayConfig struct {
	...
	// RequireProxyForAnthropic: 启用后，未配置代理的 Anthropic 请求将直接返回错误，
	// 而不是从网关出口 IP 直连 api.anthropic.com（避免所有无代理账号共用同一指纹）。
	// 默认 false 以兼容存量部署。
	RequireProxyForAnthropic bool `mapstructure:"require_proxy_for_anthropic"`
...
func setDefaults() {
	...
	viper.SetDefault("gateway.require_proxy_for_anthropic", false)
