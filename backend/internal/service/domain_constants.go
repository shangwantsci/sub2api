...
const (
	...
	SettingKeyEnableContentSafetyFilter        = "enable_content_safety_filter"         // 是否启用本地用户请求内容安全过滤（默认 true）
	SettingKeyContentSafetyGuardMode           = "content_safety_guard_mode"            // 本地内容安全过滤模式：off/warn/block（默认 block）
	SettingKeyContentSafetyLogRedactedEvidence = "content_safety_log_redacted_evidence" // 内容安全过滤日志是否记录脱敏证据片段（默认 true）
	...
	// SettingKeyClaudeCodeMimicryProfile Claude Code 伪装 profile ID（默认 2.1.206 macOS arm64）
