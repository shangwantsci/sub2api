...
func TestGatewayService_AnthropicAPIKeyPassthrough_ModelMappingPreservesOtherFields(t *testing.T) {
	...
	require.False(t, gjson.GetBytes(sentBody, "max_tokens").Exists(),
		"count_tokens 必须移除 generation-only max_tokens，避免上游 400")
}
...
func TestGatewayService_AnthropicAPIKeyPassthrough_CountTokensFiltersGenerationFields(t *testing.T) {
	...
	require.False(t, gjson.GetBytes(sentBody, "max_tokens").Exists(),
		"count_tokens does not accept generation-only max_tokens")
...
func TestGatewayService_AnthropicOAuth_NotAffectedByAPIKeyPassthroughToggle(t *testing.T) {
	...
	require.NotContains(t, getHeaderRaw(req.Header, "anthropic-beta"), claude.BetaOAuth, "OAuth 默认路径不再补齐 legacy oauth beta")
	require.Contains(t, getHeaderRaw(req.Header, "anthropic-beta"), claude.BetaThinkingTokenCount)
}
...
func TestGatewayService_AnthropicOAuthMimicHaiku_RewritesSystemAndMigratesCacheControlTTL(t *testing.T) {
	...
	body := []byte(`{"model":"claude-haiku-4-5-20251001","system":[{"type":"text","text":"Cached project instructions","cache_control":{"type":"ephemeral","ttl":"1h"}}],"messages":[{"role":"user","content":[{"type":"text","text":"hello"}]}]}`)
	...
	upstream := &anthropicHTTPUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"application/json"},
				"x-request-id": []string{"rid-oauth-haiku-cache"},
			},
	...
	account := &Account{
		ID:          302,
		Name:        "anthropic-oauth-haiku-cache",
		Platform:    PlatformAnthropic,
		Type:        AccountTypeOAuth,
		Concurrency: 1,
		Credentials: map[string]any{
...
func TestGatewayService_AnthropicSetupTokenMimicOpus_AlignsInjectedCacheControlTTL(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	body := []byte(`{"model":"claude-opus-4-8","system":[{"type":"text","text":"Cached project instructions","cache_control":{"type":"ephemeral","ttl":"1h"}}],"messages":[{"role":"user","content":[{"type":"text","text":"hello"}]}]}`)
	parsed, err := ParseGatewayRequest(NewRequestBodyRef(body), PlatformAnthropic)
	require.NoError(t, err)
	...
	upstream := &anthropicHTTPUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"application/json"},
				"x-request-id": []string{"rid-setup-opus-cache"},
			},
	...
	account := &Account{
		ID:          303,
		Name:        "anthropic-setup-opus-cache",
		Platform:    PlatformAnthropic,
	...
	require.Equal(t, "ephemeral", gjson.GetBytes(upstream.lastBody, "system.2.cache_control.type").String())
